...
int64_t GetDayOfYear(int64_t timestamp);
...
int main()
{
    while (true)
    {
        ...
        _log(INFO, "MAIN", "Sleeping for 300 seconds");

        std::this_thread::sleep_until(std::chrono::system_clock::now() + std::chrono::seconds(300));
    }
    return 0;
}
...
bool DownloadProjectTeamFiles()
{
    vprojectteamids.clear();
        ...
        for (const auto& prjs : vwhitelist)
        {
            ...
            fs::path team_file = team_file_name.c_str();
...
//                fs::remove(team_file);
            ...
            fs::path chkfile = chketagfile.c_str();
...
bool DownloadProjectRacFiles()
{
        ...
        for (const auto& prjs : vwhitelist)
        {
            ...
            fs::path rac_file = rac_file_name.c_str();
...
void testdata(const std::string& etag)
{
    std::ifstream ingzfile(etag + ".gz", std::ios_base::in | std::ios_base::binary);
